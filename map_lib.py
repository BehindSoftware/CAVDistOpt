    ...
    with open("data/cross.dettest.xml", "w") as detectors:
        ...
        for node in range(1,intersection_ID*5+1):
            if(node%5!=0):
                ...
                print('<inductionLoop id="{}" lane="{}_0" pos="{}" freq="1" length="450" file="../reports/cross.out" friendlyPos="x"/>'.format(detector_ID,lane_number+1,detector_pos), file=detectors)
                print('<inductionLoop id="{}" lane="{}_0" pos="{}" freq="1" length="450" file="../reports/cross.out" friendlyPos="x"/>'.format(detector_ID+4,(lane_number+1)%100,detector_pos), file=detectors)
                detector_ID+= 1
            else:
